...
const brandingJS = `(() => {
  ...
  function addBrandSettings(){
    ...
    card.innerHTML=`<h2 style="margin-top:0">👨‍⚕️ Doctor & Clinic Profile</h2>
      <p class="small">Set the identity for this software installation. These details are used in the dashboard, greetings, prescriptions, receipts and WhatsApp messages.</p>
      <div class="formgrid"><div class="field"><label>Doctor Name</label><input id="brandDoctorName" placeholder="e.g. Dr. Amit Patel"></div><div class="field"><label>Dental Clinic / Practice Name</label><input id="brandClinicName" placeholder="e.g. Amit Dental Clinic"></div></div>
      <div class="settings-actions"><button type="button" class="primary" id="saveBrandProfileBtn">Save Doctor & Clinic Profile</button><button type="button" class="secondary" id="restoreBrandProfileBtn">Restore Default</button></div>
      <div class="settings-note">Once saved, the doctor/clinic identity replaces the original doctor's name in generated greetings and WhatsApp messages. The logo remains optional and can be changed below.</div>`;
