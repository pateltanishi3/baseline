...
const brandingJS = `(() => {
  ...
  function brandProfile(){db.settings=db.settings||{};return {doctor:String(db.settings.doctorName||DEFAULT_DOCTOR).trim()||DEFAULT_DOCTOR,clinic:String(db.settings.clinicName||DEFAULT_CLINIC).trim()||DEFAULT_CLINIC};}
  function brandDoctor(){return brandProfile().doctor;} function brandClinic(){return brandProfile().clinic;}
  function applyBrandLogo(src){const use=src&&String(src).startsWith('data:')?src:APP_LOGO_DATA;document.querySelectorAll('.brand-logo').forEach(img=>img.src=use);const p=document.getElementById('settingsLogoPreview');if(p)p.src=use;}
  if(typeof applyClinicLogo==='function'){applyClinicLogo=function(src){applyBrandLogo(src);};applyBrandLogo(db.settings?.logo||'');}
  function refreshBranding(){const p=brandProfile();document.title=p.clinic+' — Dental Patient Management Software';const d=document.getElementById('dashClinicName');if(d)d.textContent=p.clinic;const g=document.getElementById('dashGreeting');if(g)g.textContent='Good Morning, '+p.doctor+' 👋';applyBrandLogo(db.settings?.logo||'');}
  function addBrandSettings(){if(document.getElementById('clinicProfileSettings')){refreshBranding();return;}const section=document.getElementById('settings');if(!section)return;const card=document.createElement('div');card.className='card';card.id='clinicProfileSettings';card.innerHTML=`<h2 style="margin-top:0">👨‍⚕️ Doctor & Clinic Profile</h2><p class="small">Set the identity for this software installation. These details are used in the dashboard, greetings, prescriptions, receipts and WhatsApp messages.</p><div class="formgrid"><div class="field"><label>Doctor Name</label><input id="brandDoctorName" placeholder="e.g. Dr. Amit Patel"></div><div class="field"><label>Dental Clinic / Practice Name</label><input id="brandClinicName" placeholder="e.g. Amit Dental Clinic"></div></div><div class="settings-actions"><button type="button" class="primary" id="saveBrandProfileBtn">Save Doctor & Clinic Profile</button><button type="button" class="secondary" id="restoreBrandProfileBtn">Restore Default</button></div><div class="settings-note">Once saved, the doctor/clinic identity replaces the original doctor's name in generated greetings and WhatsApp messages. The logo remains optional and can be changed below.</div>`;const firstCard=section.querySelector('.card');firstCard?.before(card);const d=document.getElementById('brandDoctorName'),c=document.getElementById('brandClinicName');d.value=brandDoctor();c.value=brandClinic();document.getElementById('saveBrandProfileBtn').onclick=()=>{const doctor=d.value.trim(),clinic=c.value.trim();if(!doctor||!clinic){alert('Please enter both Doctor Name and Dental Clinic / Practice Name.');return;}db.settings=db.settings||{};db.settings.doctorName=doctor;db.settings.clinicName=clinic;saveDB();refreshBranding();alert('Doctor and clinic profile saved.');};document.getElementById('restoreBrandProfileBtn').onclick=()=>{db.settings=db.settings||{};db.settings.doctorName=DEFAULT_DOCTOR;db.settings.clinicName=DEFAULT_CLINIC;saveDB();d.value=DEFAULT_DOCTOR;c.value=DEFAULT_CLINIC;refreshBranding();};refreshBranding();}
  function dynamicReceiptFooterJpeg(){const canvas=document.createElement('canvas');canvas.width=800;canvas.height=120;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,800,120);ctx.strokeStyle='#b52a2a';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(25,12);ctx.lineTo(775,12);ctx.stroke();ctx.fillStyle='#18364f';ctx.textAlign='center';ctx.font='bold 22px Segoe UI, Arial';ctx.fillText(brandClinic(),400,55);ctx.fillStyle='#4b5563';ctx.font='16px Segoe UI, Arial';ctx.fillText(brandDoctor(),400,82);ctx.fillStyle='#2f8b8c';ctx.font='bold 13px Segoe UI, Arial';ctx.fillText('Dental Patient Management Software',400,104);return canvas.toDataURL('image/jpeg',0.9).split(',')[1];}
  if(typeof buildReceiptBlob==='function'){const source=buildReceiptBlob.toString().replace(/pdfAscii\('Dr\. Amit J\. Patel'\)/g,'pdfAscii(brandDoctor())').replace(/base64Bytes\(RECEIPT_FOOTER_JPEG\)/g,'base64Bytes(dynamicReceiptFooterJpeg())');try{buildReceiptBlob=eval('('+source+')');}catch(e){console.warn('Dynamic receipt branding unavailable',e);}}
  ...
  const originalRenderDash=renderDash;renderDash=function(){originalRenderDash();refreshBranding();};const originalShow=show;show=function(id,btn){originalShow(id,btn);if(id==='settings'){setTimeout(addBrandSettings,0);setTimeout(refreshBranding,0);}};addBrandSettings();refreshBranding();
})();`

func buildHTML() string {
    svg := "data:image/svg+xml;base64," + base64.StdEncoding.EncodeToString(appIconSVG)
    js := strings.ReplaceAll(brandingJS, "__APP_LOGO_DATA__", fmt.Sprintf("%q", svg))
